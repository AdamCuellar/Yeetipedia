        ...
        if(!(username.text == "" || password.text == ""))
        {
            ...
            postRequest(username: user, password: pass) { (result, error) in
                if result?["state"] as! Int == 1
                {
                    ...
                    DispatchQueue.main.async {
                        let alert = UIAlertController(title: "Error", message: "The username you've entered does not exist.", preferredStyle: .alert)
                    ...
                    DispatchQueue.main.async {
                        let alert = UIAlertController(title: "Error", message: "The password you've entered is not correct.", preferredStyle: .alert)
